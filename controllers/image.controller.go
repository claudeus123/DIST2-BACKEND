...
func ImageUpload(context *fiber.Ctx) error {
    ...
    err = context.SaveFile(file, fmt.Sprintf("./uploads/%d-%s.jpg", userID, orderStr ))
    ...
    data := map[string]interface{}{
        "imageName": fmt.Sprintf("./uploads/%d-%s.jpg", userID, orderStr ),
        "header":    file.Header,
        "size":      file.Size,
    }
...
func ImageServe(context *fiber.Ctx) error{
	...
	if err != nil {
		return context.JSON(fiber.Map{"status": 400, "message": "Invalid userID", "data": nil})
	}
...
// 	// Obtener ID de los parámetros de la ruta
	...
	if _, err := os.Stat(imagePath); os.IsNotExist(err) {
		...
		response := fiber.Map{
			"user":  user,
			"image": "/static/" + fmt.Sprint(user.Id) + "-" + imageID + ".jpg",
		}
